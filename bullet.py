...
class Bullet_1(pygame.sprite.Sprite):
    def __init__(self, x, y, x_place, y_place):
    ...
    def update(self, keys):
        from main import game, all_sprites_zobies, GRID, WIDTH, HEIGHT
        if game == False:self.kill()    #вышли из игры - удаляем спрайт

        if pygame.sprite.spritecollideany(self, all_sprites_zobies): self.kill()    #если попали в зомби - удаляем спрайт
        if abs(self.rect.center[0] - self.place_center[0]) <= 20 and abs(self.rect.center[1] - self.place_center[1]) <= 20: self.ok = True
        if self.ok == False:
            speed_curs_x =  (self.place_center[0] - self.rect.center[0])//(GRID*2)
            speed_curs_y =(self.place_center[1] - self.rect.center[1])//(GRID*2)
